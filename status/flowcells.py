...
class ReadsTotalCalculator:
    """Helper class for calculating total reads data for a given query.

    This class handles the logic of fetching and aggregating read count
    data from the database for samples matching the query string.
    """

    @staticmethod
    def calculate_reads_data(cloudant, query):
        """Calculate reads data for a given query.

        Args:
            cloudant: The cloudant database connection
            query: The sample query string

        Returns:
            OrderedDict: Ordered dictionary with sample data and read counts
        """
        if not query:
            return OrderedDict()
        ...
        # Fetch lane clusters data
        xfc_view = cloudant.post_view(
            db="x_flowcells",
            ddoc="samples",
            view="lane_clusters",
            start_key=query,
        ...
        bioinfo_view = cloudant.post_view(
            db="bioinfo_analysis",
            ddoc="latest_data",
            view="sample_id",
...
class ReadsTotalHandler(SafeHandler):
    ...
    def get(self, query):
        ...
        # Use the helper class to calculate reads data
        readsdata = ReadsTotalCalculator.calculate_reads_data(
            self.application.cloudant, query
        )
...
class ReadsTotalDataHandler(SafeHandler):
    """Serves read count data as JSON for a given query.

    Loaded through /api/v1/reads_total/([^/]*)$ url
    """
    ...
    def get(self, query):
        ...
        self.write(json.dumps(readsdata))
